...
class Point(object):
    ...
    def __repr__(self):
        x = self.x
        y = self.y
        return 'Point(' + format(x) + ', ' + format(y) + ')'
    ...
    def move_to(self, x, y):
        ...
        self.y = y
        self.moves += 1
        self.travel_distance += math.sqrt(parax ** 2 + paray ** 2)

    def move_by(self, x, y):
        self.x += x
        self.y += y
        self.moves += 1
        self.travel_distance += math.sqrt(x ** 2 + y ** 2)
    ...
    def closer_to(self, point1, point2):
        x1 = self.get_distance_from(point1)
        x2 = self.get_distance_from(point2)
        if x1 < x2:
            return point1
        else:
            return point2
